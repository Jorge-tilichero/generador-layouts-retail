...
def dibujar_layout_oxxo_v21(conf):
    ...
    def registrar_obj(x, y, w, h, color, texto="", rot=0, alpha=1.0, font=6, z=5, tipo="Fisico", name="Objeto", txt_col='black', weight='normal'):
        ...
        if tipo == "Fisico":
            ...
            c2, n2 = colisiona(x, y, w, h, obs_pasillos) # Blindaje de pasillos
            if c1: choca, obj_chocado = True, n1
            elif c2: choca, obj_chocado = True, n2
        elif tipo == "Pasillo":
            choca, obj_chocado = colisiona(x, y, w, h, obs_fisicos) # Pasillos solo chocan con muros/muebles previos
    ...
    # ==========================================
    # 1. BODEGA PARAMÉTRICA (Control Total)
    # ==========================================
    a_op = 0
    if conf['t_bodega']:
    ...
    # ==========================================
    # 3. CHECKOUT (Rotación y Pasillos)
    # ==========================================
    if conf['t_check']:
        ...
        if rot_c == 0: 
            ...
            registrar_obj(xc, yc + PROF_CONTRA, w_chk, PROF_CAJERO, '#EAEDED', "P. CAJERO (1m)", tipo="Pasillo", name="Pasillo Cajero")
    ...
    # ==========================================
    # 4. CUARTO FRÍO (Rotación y Escuadra)
    # ==========================================
    if conf['t_frio']:
    ...
    # ==========================================
    # 6. PERIMETRALES (Por Muro Individual)
    # ==========================================
...
with st.sidebar:
    ...
    # Placeholders para actualizar después del dibujo
    kpi_bod = st.empty()
    kpi_exh = st.empty()
    kpi_nav = st.empty()
...
with col_info:
    ...
    with st.expander("3. Red de Pasillos (Blindaje)", expanded=False):
        t_pasillos = st.checkbox("Habilitar Blindaje de Pasillos", value=True)
